...
function App() {
  ...
  const handleSelectChange = (event) => {
    ...
    let selectedCount = Data.filter(beast => beast.horns === numberOfHorns)
    console.log(selectedCount)
    event.target.value === "" ? setBeastData(Data) : setBeastData(selectedCount)
  };

  return (
    <div className="App">
      <Header />
      <FilterDropdown handleSelectChange={handleSelectChange}/>
      <Main data={beastData} handleModal={handleModal} />
      {showModal && (
        <SelectedBeast
          modalContent={modalContent}
          closeModal={closeModal}
...
export default App;
  ...
  // if u see this in the future message me 
  // this is super complex, for no reason. 
  // function handleFav(title) {
  //   setFavCounts((prevFavCounts) => ({
  //     ...prevFavCounts,
  //     [title]: (prevFavCounts[title] || 0) + 1, // hashmap??? 
  //   }));
  // }